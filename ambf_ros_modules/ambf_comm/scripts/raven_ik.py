...
def dhvalue_to_joint(dhvalue, gangle, arm):
    for i in range(RAVEN_JOINTS - 1):
        if i != 2:
            ...
            joint[i] = dhvalue[i]
    joint = apply_joint_limits(joint)
    return joint

def apply_joint_limits(joint):
    if joint.size() != RAVEN_JOINTS:
        ROS_ERROR("Wrong vector length to apply joint limit check.")
        ...
        if ikcheck[i] == True:
            for j in range(RAVEN_JOINTS - 1):
                if j == 2:
                    ...
                    diff = float(iksol[i][j] - curr_jp[i])
                    while diff > m.pi:
                        diff -= 2 * m.pi
                    while diff < -m.pi:
                        diff += 2 * m.pi
